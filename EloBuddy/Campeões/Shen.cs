...
namespace EloBuddy
{
    class Shen
    {
        ...
        private static void Game_OnStart(EventArgs args)
        {
            ...
            foreach (var spell in Player.Instance.Spellbook.Spells.Where( i =>i.Name.ToLower().Contains("smite") &&  (i.Slot == SpellSlot.Summoner1 || i.Slot == SpellSlot.Summoner2)))
            { Smite = spell.Slot;}
        ...
        public static void LoadingSkin()
        {
            if (Menu.Get<KeyBind>("SkinLoad").CurrentValue)
            {
                ...
                {
                    var SkinHackSelect = Shen.Menu["SkinID"].DisplayName;
        ...
        private static void Game_OnDraw(EventArgs args)
        {
            ...
            switch (SkinHackSelect)
            {
                ...
                case "Classic": color = Color.MediumVioletRed; break;
                case "SkinHack 1":  color = Color.DeepSkyBlue; break;
                case "SkinHack 2":color = Color.OrangeRed; break;
                case "SkinHack 3":color = Color.MediumSeaGreen; break;
                case "SkinHack 4": color = Color.DarkRed; break;
                case "SkinHack 5": color = Color.OrangeRed; break;
                case "SkinHack 6": color = Color.BlueViolet; break;
            }
            ...
            if (DrawText)
            {
                var SelectModeTaunt = Shen.Menu["TauntModes"].DisplayName;
                ...
                Text.Draw();
                var SelectModeShield = Shen.Menu["ShieldMode"].DisplayName;
        ...
        {
            if (target.IsValidTarget(Player.Instance.GetAutoAttackRange() + 200))
            {
                if (Menu["UseItemAtk"].Cast<CheckBox>().CurrentValue)
                {
                    if (Tiamat.IsReady()) Tiamat.Cast(); //Orbwalker.ResetAutoAttack();
                    if (TitanicHydra.IsReady()) TitanicHydra.Cast(); //Orbwalker.ResetAutoAttack();
                    if (Hydra.IsReady()) Hydra.Cast() ; //Orbwalker.ResetAutoAttack();
                }
            }
            return;
        }
        public static void Combo()
        {
            ...
            foreach (var Objeto in ObjectManager.Get<Obj_AI_Minion>().Where(o => o.IsValid && !o.IsDead && o.BaseSkinName == "ShenSpirit"))
            {
                            ...
                            {
                                {
                                    ...
                                    switch (SelectModeShield)
                                    {
                                        case "[ Use Logic Count Enemy ]":
                                            ...
                                            { W.Cast(); } break;
                                        case "[ Use Aways ]":
                                            if (Q.IsOnCooldown) W.Cast(); break;
                                    }
                                }
                            }
                    }
                ...
                if (Menu["ECombo"].Cast<CheckBox>().CurrentValue)
                {
                    ...
                    {
                        case "[ Use Max Range ]":if (Inimigo.IsValidTarget(600) && E.IsReady()) E.Cast(Inimigo.Position); break;
                        case "[ Use Fast ]": if (E.IsReady()) E.Cast(Inimigo.Position);break;
                    }
                }
            }
        ...
        public static void LaneClear()
        {
            ...
            {
                foreach (var source in EntityManager.MinionsAndMonsters.EnemyMinions.Where(a => a.IsValid && a.IsValidTarget(250) && !a.IsDead)) Q.Cast();
            }
        }
        public static void JungleClear()
        {
            if (Menu["QJungle"].Cast<CheckBox>().CurrentValue)
            {
                ...
                if (Q.IsReady() && jg.IsValidTarget(250)) Q.Cast();
            }
        }
        public static void Execute()
        {
                var creep = EntityManager.MinionsAndMonsters.GetJungleMonsters(Player.Instance.Position, 800).Where(x => x.BaseSkinName == "SRU_Dragon" || x.BaseSkinName == "SRU_Baron");
                foreach (var x in creep.Where(y => Player.Instance.Distance(y.Position) <= Player.Instance.BoundingRadius + 500 + y.BoundingRadius))
                {
                    if (x != null && x.Health <= GetSmiteDamage()) Player.Instance.Spellbook.CastSpell(Smite, x);
                }
        }
