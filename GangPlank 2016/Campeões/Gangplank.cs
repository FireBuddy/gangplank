...
namespace UnrealSkill
{
    ...
    internal class Gangplank
    {
        ...
        {
            InitGangPlank();
            Chat.Print("|| GangPlank Load || Credit <font color='#00CC00'><b>Soresu</b></font> <b><font color='#002eff'> & </font><font color='#FF0000'>UnrealSkill99</font></b> || Challenger Script To EB GP", Color.White);
            Chat.Print("|| GangPlank Load || Credit <font color='#00CC00'><b>Soresu</b></font> <b><font color='#002eff'> & </font><font color='#FF0000'>UnrealSkill99</font></b> || Challenger Script To EB GP", Color.White);
        ...
        public static void Combo()
        {
            ...
            if (EExtendido && Q.IsReady() && E.IsReady())
            {
                ...
                if (Prediction.HitChance >= HitChance.Low)
                {
                    ...
                    {
                        if (ModoBExtended == "☑ 1 Profissional")//Modo Profissional
                        {
                        ...
                        if (ModoBExtended == "☑ 2 Advanced")//Modo Avançado
                        {
                                ...
                                if (detoneateTargetBarrelSeconds.Any())
                                {
                                    foreach (var detoneateTargetBarrelSecond in detoneateTargetBarrelSeconds)
                                    {
                                        if (detoneateTargetBarrelSecond.Distance(target) <= BarrelExplosionRange && target.Distance(detoneateTargetBarrelSecond.Position) <= BarrelExplosionRange)
                                        {
                                            ...
                                            else if (detoneateTargetBarrel.Distance(Player.Instance.Position) <  Player.Instance.GetAutoAttackRange)
                                            {
        ...
        public static void InitMenu(EventArgs args)
        {
            ...
            Menu.AddSeparator(2);
            Menu.AddGroupLabel("GangPlank Logs" + System.Environment.NewLine);
            Menu.AddSeparator(2);
            Menu.AddLabel("✖ Script: BanSharp To EloBuddy" + System.Environment.NewLine);
            Menu.AddLabel("✖ Crédit: Soresu /  UnrealSkill99");
